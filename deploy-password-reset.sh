...
USER_COUNT=$(sqlite3 server/database.sqlite "SELECT COUNT(*) FROM users;")
ADMIN_COUNT=$(sqlite3 server/database.sqlite "SELECT COUNT(*) FROM admin_users;" 2>/dev/null || echo "0")
echo "   Users: $USER_COUNT"
echo "   Admins: $ADMIN_COUNT"

# Step 5: Run database migrations
echo "🗄️  Running database migrations..."
sqlite3 server/database.sqlite < migrations/001_password_resets.sql
sqlite3 server/database.sqlite < migrations/admin/001_admin_password_resets.sql
echo "   ✓ Migrations complete"

# Step 6: Verify data integrity (post-migration check)
echo "🔍 Verifying data integrity..."
USER_COUNT_AFTER=$(sqlite3 server/database.sqlite "SELECT COUNT(*) FROM users;")
ADMIN_COUNT_AFTER=$(sqlite3 server/database.sqlite "SELECT COUNT(*) FROM admin_users;" 2>/dev/null || echo "0")
...
sqlite3 server/database.sqlite "SELECT COUNT(*) FROM password_resets;" > /dev/null
sqlite3 server/database.sqlite "SELECT COUNT(*) FROM admin_password_resets;" > /dev/null
