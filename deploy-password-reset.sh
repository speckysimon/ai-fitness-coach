...
cp server/fitness-coach.db server/fitness-coach.db.backup.$(date +%Y%m%d_%H%M%S) 2>/dev/null || echo "Admin DB not found, skipping backup"
...
ADMIN_COUNT=$(sqlite3 database.sqlite "SELECT COUNT(*) FROM admin_users;" 2>/dev/null || echo "0")
...
USER_COUNT_AFTER=$(sqlite3 database.sqlite "SELECT COUNT(*) FROM users;")
ADMIN_COUNT_AFTER=$(sqlite3 database.sqlite "SELECT COUNT(*) FROM admin_users;" 2>/dev/null || echo "0")
...
sqlite3 database.sqlite "SELECT COUNT(*) FROM password_resets;" > /dev/null
sqlite3 database.sqlite "SELECT COUNT(*) FROM admin_password_resets;" > /dev/null
